...
def calc_reward(outputs):
    ...
    J = tf.concat([tf.log(p_y + 1e-5) * onehot_labels_placeholder, tf.log(p_loc + 1e-5) * R], 1)
    J = tf.reduce_sum(J, 1)
...
with tf.Graph().as_default():
    ...
    if load_path is not None and ckpt and ckpt.model_checkpoint_path:
        ...
        init = tf.global_variables_initializera()
